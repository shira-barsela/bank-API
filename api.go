...
var loginUsersTokens map[int]string
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	loginUsersTokens[authenticatedUser.ID] = tokenString
}
...
func AccountsHandler(w http.ResponseWriter, r *http.Request, claims *Claims) {
	...
	if r.Method == http.MethodGet {
		// added a check for admin, so that regular users couldn't see the listAccounts
		if claims.Role != "admin" {
			http.Error(w, "Unauthorized", http.StatusForbidden)
			return
		}
...
func getBalance(w http.ResponseWriter, r *http.Request, claims *Claims) {
	...
	// check if the user that sent the request owns the account / his the admin.
	// the ownership check is by loginUsersTokens map.
	if loginUsersTokens[uid] != r.Header.Get("Authorization") && claims.Role != "admin" {
		http.Error(w, "Unauthorized", http.StatusForbidden)
		return
	}
...
func depositBalance(w http.ResponseWriter, r *http.Request, claims *Claims) {
	...
	// check if the user that sent the request owns the account.
	// the ownership check is by loginUsersTokens map.
	if loginUsersTokens[body.UserID] != r.Header.Get("Authorization") {
		http.Error(w, "Unauthorized", http.StatusForbidden)
		return
	}
